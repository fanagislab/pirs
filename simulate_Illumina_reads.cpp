 ...
 */
PARAMETER InputParameter ={150,350,-1,0,0,0,1,0,33,0,1,5,-1,"","","","","","Illumina"};
...
void SimReads_Usage(){
	...
	cout<<"sequencer, such as insertsize distribution, sequencing error(substitution), and quality score. ";
	...
	cout<<"This program simulates Illumina sequencing error and quality score according to the empirical distribution profile. ";
	...
	cout<<"\t-s  <string>  User-defined Base-calling profile, if not set, use the default Base-calling profile depended on read length" <<endl;
	cout<<"\t-l  <int>     read_len,set length of read,read1 and read2 have the same length, only support values 50,100,150,200,250,300,350,400,450,500,550,600, default:"<<InputParameter.Read_length<<endl;
...
//	cout<<"\t-E  <int>     mode of read-end masking by EAMSS algorithm, 0:none, 1:Quality=2, 2:lowercase Base. default:"<< InputParameter.Mask_quality_mode<<endl;
	...
	cout<<"\t   ./pirs simulate -i path/my_genome.fa  -l 300  -m 500 -x 40 -o my_genome \n"<<endl;
	cout << "\n\nAltert: This program uses the starting run-time to create random numbers, so jobs of reads-simulation with the same reference genome should not run at the same time" << endl;
...
int simulate_Illumina_reads(int argc, char *argv[])
{
	...
	if(InputParameter.Read_length != 50 && InputParameter.Read_length != 100 && InputParameter.Read_length != 150 && InputParameter.Read_length != 200 && InputParameter.Read_length != 250 && InputParameter.Read_length != 300 && InputParameter.Read_length != 350 && InputParameter.Read_length != 400 && InputParameter.Read_length != 450 && InputParameter.Read_length != 500 && InputParameter.Read_length != 550 && InputParameter.Read_length != 600){cerr<<"Error: read length should be set to values 50,100,150,200,250,300,350,400,450,500,550,600, please check option -l !"<<endl;exit(-1);}
	...
	{	InputParameter.BaseCalling_profile = "/public/agis/fanwei_group/fanwei/code/pirs_simple/Profiles/humNew.PE" + boost::lexical_cast<std::string>(InputParameter.Read_length) + ".matrix";
		cerr << "Using default base-calling profile: " << InputParameter.BaseCalling_profile << endl;
	}else
	{
		cerr << "Using user-defined base-calling profile: " << InputParameter.BaseCalling_profile << endl;
	}
